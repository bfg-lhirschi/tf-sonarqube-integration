...
locals {
  ...
  java_build_run   = local.java_build_tool == "gradle" ? "./gradlew build sonarqube --info" : (local.java_build_tool == "maven" ? "mvn -B verify org.sonarsource.scanner.maven:sonar-maven-plugin:sonar -Dsonar.host.url=${var.sonar_host_url} -Dsonar.login=${var.sonar_token}" : null) 
}
